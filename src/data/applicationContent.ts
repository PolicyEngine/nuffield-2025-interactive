...
This local variation affects households in two ways that current analysis misses. First, distributional: Stroud District Council offers up to 100% support [@stroud_ctr_2025], while Dudley requires 60% minimum payments [@dudley_ctr_2025]—an identical household pays nothing in one area and over half their bill in another. Second, work incentives: CTR schemes create marginal tax rates that stack on top of Universal Credit's 55% taper, income tax, and National Insurance. Policy debate has focused on the childcare cliff at £100k and UC taper effects, yet CTR—which varies by council and can add 20-40% to effective marginal rates—is absent from these analyses because no tool models local variation. The combination can push effective marginal rates above 80% for some low-income workers, creating poverty traps that differ dramatically by postcode. Local journalists lack tools to compare their council's scheme to neighbours. Councillors considering reforms cannot model work incentive effects. This hidden variation receives little scrutiny because the information is inaccessible.

We will build a council tax policy platform where policymakers and the public can understand how CTR schemes operate, compare rules across authorities, and design reforms with fiscal and distributional analysis. Users select their local authority to see how their council's rules compare to others, what households like theirs would pay under different schemes, and what reforms would mean for local budgets and bills. This gives local journalists, councillors, and advocacy groups analytical power currently available only to central government.
